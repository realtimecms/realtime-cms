...
module.exports = function(service, definition, cms, client) {
  ...
  for(let actionName in definition.actions) {
    ...
    if(access) {
      try {
        if(!access({ visibilityTest: true }, { client, service, action, visibilityTest: true })) {
  ...
  for(let viewName in definition.views) {
    ...
    if(access) {
      try {
        if(!access({ visibilityTest: true }, { client, service, view, visibilityTest: true })) {
  ...
  for(let modelName in definition.models) {
    ...
    if(access) {
      try {
        if(!access({ visibilityTest: true }, { client, service, model, visibilityTest: true })) {
