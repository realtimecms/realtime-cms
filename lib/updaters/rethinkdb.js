...
module.exports = async function(changes, service, cms, force) {
  ...
  async function createIndex(tableName, indexName, index, indexList) {
    ...
    if(indexList.indexOf(indexName) != -1) {
      ...
      await r.table(tableName).indexDrop(indexName).run(db)
    ...
    if(index.property.constructor == Array) { // compound index
      await r.table(tableName).indexCreate(indexName, index.property.map(prop => rowPointer(prop)), options).run(db)
    } else {
      await r.table(tableName).indexCreate(indexName, rowPointer(index.property), options).run(db)
    }
