...
module.exports = async function(changes, service, cms, force) {
  ...
  async function createIndex(tableName, indexName, index, indexList) {
    ...
    if(index.property.constructor == Array) { // compound index
      await r.table(tableName).indexCreate(indexName, index.property.map(prop => r.row(prop)), options).run(db)
    } else {
      await r.table(tableName).indexCreate(indexName, r.row(index.property), options).run(db)
    }
