...
module.exports = async function(changes, module, cms, force) {
  const generateTableName = (modelName) => {
    return module.name+"_"+modelName
  }
    ...
    switch(change.operation) {
      ...
      case "renameIndex": {
        ...
        if(indexList.indexOf(change.name) != -1) {
          ...
          await createIndex(tableName, module.models[change.model].indexes[change.name])
        } else {
